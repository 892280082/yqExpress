...
angular.module("controller.main",[
    ...
    ,function($scope,showCtrl,dataService,FileUploader,pageResult,$window){
        ...
        $scope.cateSelect = [{type:3,name:'文章',url:"/back/toArticlePage"}]
        ...
        dataService.getConfig()
            .success(function(data){
                if(data.err){
                    ...
                    $scope.webConfig = data.result || {articles:[]};
                }
            }).error(function(data){
                alert("获取错误");
            })
        ...
        $scope.removeBanner = function(pojo){
            ...
            if(!tFlag)
                return false;
            $scope.webConfig.articles.remove(pojo);
        }

        $scope.bannerMove = function(currentIndex,flag){
            var oriPojo = $scope.webConfig.articles[currentIndex];
            var targetPojo = $scope.webConfig.articles[currentIndex+flag];
            $scope.webConfig.articles[currentIndex] = targetPojo;
            $scope.webConfig.articles[currentIndex + flag] = oriPojo;
        }
