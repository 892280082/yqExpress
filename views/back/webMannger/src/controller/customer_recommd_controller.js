...
angular.module("controller.main",[
    ...
    ,function($scope,showCtrl,dataService,FileUploader,pageResult,$window){
        ...
        $scope.insertBannerArray = function(){
            ...
            $scope.webConfig.customers[index]=insertPojo;
            $scope.$apply();
        }
        ...
        dataService.getConfig()
            .success(function(data){
                if(data.err){
                    ...
                    $scope.webConfig = data.result || {customers:[]};
                }
            }).error(function(data){
                alert("获取错误");
            })
        ...
        $scope.addBanner = function(){
            $scope.webConfig.customers.push({});
        }
        ...
        $scope.removeBanner = function(pojo){
            ...
            if(!tFlag)
                return false;
            $scope.webConfig.customers.remove(pojo);
        }

        $scope.bannerMove = function(currentIndex,flag){
            var oriPojo = $scope.webConfig.customers[currentIndex];
            var targetPojo = $scope.webConfig.customers[currentIndex+flag];
            $scope.webConfig.customers[currentIndex] = targetPojo;
            $scope.webConfig.customers[currentIndex + flag] = oriPojo;
        }
        ...
        $scope.saveOrUpdate = function(){
            ...
            if(!$scope.webConfig._id){
                ...
                //更新
                dataService.updateCustomer($scope.webConfig,'customers')
                    .success(function(data){
                        if(!data.err){
                            alert("更新成功");
                        }else{
