...
angular.module("controller.product_recommd_controller",[
    "ng.ueditor"
]).controller('product_recommd_controller',['$scope','showCtrl','dataService','FileUploader','pageResult','$window'
    ,function($scope,showCtrl,dataService,FileUploader,pageResult,$window){
        /*********************注册show service**************************/
        $scope.show = showCtrl;
        $scope.show.$regist('cusadd',['cusadd'],true);

        $scope.cateSelect = [{type:2,name:'创品',url:'/back/toProducs'}];
        ...
        $scope.insertBannerArray = function(){
            ...
            if(type === 1){
                ...
                insertPojo.picUrl = pojo.imgBigUrl;
            }else if(type === 3){
                insertPojo.title = pojo.title;
            ...
            $scope.webConfig.products[index]=insertPojo;
            $scope.$apply();
        }
        ...
        dataService.getConfig()
            .success(function(data){
                if(data.err){
                    ...
                    $scope.webConfig = data.result || {products:[]};
                }
            }).error(function(data){
                alert("获取错误");
            })
        ...
        $scope.removeBanner = function(pojo){
            ...
            if(!tFlag)
                return false;
            $scope.webConfig.products.remove(pojo);
        }

        $scope.bannerMove = function(currentIndex,flag){
            var oriPojo = $scope.webConfig.products[currentIndex];
            var targetPojo = $scope.webConfig.products[currentIndex+flag];
            $scope.webConfig.products[currentIndex] = targetPojo;
            $scope.webConfig.products[currentIndex + flag] = oriPojo;
        }
        /***********************添加或编辑用户页面*****************************/
